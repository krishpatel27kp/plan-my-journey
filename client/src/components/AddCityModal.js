...
export function renderAddCityModal({ tripId, currentStopsCount = 0, onCityAdded, onClose }) {
  ...
  modal.innerHTML = `
    <div class="card animate-fade-in" style="width: 100%; max-width: 580px; height: 88vh; max-height: 88vh; display: flex; flex-direction: column; background: #ffffff; border-radius: 28px 28px 0 0; border: 1px solid var(--color-border); box-shadow: 0 -15px 50px rgba(15,23,42,0.35); padding: 0; position: relative; overflow: hidden;">
      
      <!-- Top Grab Bar / Drag Handle -->
      <div style="display: flex; justify-content: center; padding-top: 0.75rem; background: #ffffff; flex-shrink: 0;">
        <div style="width: 46px; height: 5px; background: #cbd5e1; border-radius: 9999px;"></div>
      </div>

      <!-- Header: Add Destination + Close Button -->
      <div style="padding: 0.75rem 1.5rem 0.5rem 1.5rem; display: flex; justify-content: space-between; align-items: center; background: #ffffff; flex-shrink: 0;">
        <div>
      ...
      <div style="padding: 0.5rem 1.5rem 0.75rem 1.5rem; background: #ffffff; flex-shrink: 0;">
      ...
      <div style="padding: 0 1.5rem 0.85rem 1.5rem; display: flex; gap: 0.5rem; overflow-x: auto; background: #ffffff; border-bottom: 1px solid #f1f5f9; flex-shrink: 0;" id="category-pills">
        ${CATEGORIES.map(c => `
          <button 
            class="btn-cat-pill" 
            data-cat="${c.id}" 
      ...
      <!-- Destinations Scroll List (Scrollable Area) -->
      <div style="flex: 1 1 auto; min-height: 0; overflow-y: auto; -webkit-overflow-scrolling: touch; padding: 1.25rem 1.5rem 5rem 1.5rem; display: flex; flex-direction: column; gap: 1.25rem; background: #f8fafc;" id="dest-list">
    ...
    try {
      ...
      if (Array.isArray(serverCities) && serverCities.length > 0) {
        rawCities = serverCities.map(sc => {
          ...
          if (nameLower.includes('goa') || nameLower.includes('pondicherry') || nameLower.includes('bali') || nameLower.includes('sydney') || nameLower.includes('rio') || nameLower.includes('alleppey')) {
            category = 'Beaches';
          } else if (nameLower.includes('manali') || nameLower.includes('leh') || nameLower.includes('rishikesh') || nameLower.includes('cape town')) {
            category = 'Nature';
          }
          ...
          return {
            ...
            name: sc.name,
            country: sc.country,
            popularity: sc.popularity || 90,
            category,
            costPerDay: costMap[sc.costIndex] || '₹3,500',
            description: `Explore scenic views, cultural sights, and local dining in ${sc.name}.`,
            imageUrl: sc.imageUrl || 'https://images.unsplash.com/photo-1512343879784-a960bf40e7f2?auto=format&fit=crop&w=800&q=80'
          };
        });
      }
    } catch (e) {
      console.warn('Cities API fallback');
    }

    if (!rawCities || rawCities.length === 0) {
      rawCities = [
        {
          id: '4fd95dee-a248-4058-be85-eeef6723cbfa',
          name: 'Goa',
          country: 'India',
          popularity: 94,
          category: 'Beaches',
        ...
        {
          id: 'b70f6159-14f4-465b-9f1d-68e6863c9177',
          name: 'Manali',
          country: 'India',
          popularity: 88,
          category: 'Nature',
        ...
        {
          id: '94abacff-c04d-4cbd-8bda-45ec4d05f691',
          name: 'Jaipur',
          country: 'India',
          popularity: 91,
          category: 'Culture',
        ...
        {
          id: 'db58ec67-6a64-4141-a83f-f7ba74201db9',
          name: 'Leh',
          country: 'India',
          popularity: 91,
          category: 'Nature',
        ...
        {
          id: '81410662-67c9-4102-b75b-07c483bd8d7a',
          name: 'Paris',
          country: 'France',
          popularity: 98,
          category: 'Culture',
        ...
        {
          id: '2e7c49c2-eca0-4cfd-ae8b-d4ad5e06fb4e',
          name: 'Tokyo',
          country: 'Japan',
          popularity: 98,
          category: 'Culture',
        ...
        {
          id: '7a75381b-82ad-4c17-a882-5f51d6107066',
          name: 'Rishikesh',
          country: 'India',
          popularity: 89,
          category: 'Nature',
        ...
        {
          id: '66637125-90ae-4dda-8fcf-0226bd8666ab',
          name: 'Pondicherry',
          country: 'India',
          popularity: 85,
          category: 'Beaches',
  ...
  function renderDestList() {
      ...
      destList.innerHTML = `
        <div style="text-align: center; padding: 3rem 1.5rem; background: #ffffff; border-radius: 20px; border: 1px dashed #cbd5e1;">
          ...
          <p style="color: #64748b; font-size: 0.9rem;">
            Try selecting "All" or searching for another city name.
          </p>
        </div>
      `;
      return;
    ...
    destList.innerHTML = filtered.map(city => {
      ...
      return `
        <div class="card dest-card animate-fade-in" style="padding: 0; overflow: hidden; border: 1.5px solid var(--color-border); border-radius: 20px; box-shadow: var(--shadow-sm); background: #ffffff; display: flex; flex-direction: column; margin-bottom: 0.25rem;">
          
          <!-- Image Banner with Badges -->
          <div style="position: relative; height: 155px; width: 100%; overflow: hidden; background: #0f172a;">
            <img 
            ...
            <div style="position: absolute; top: 0.75rem; left: 0.75rem; display: flex; gap: 0.4rem; z-index: 2;">
              <span class="badge" style="background: rgba(255,255,255,0.95); backdrop-filter: blur(6px); color: #006d64; font-weight: 800; font-size: 0.75rem; padding: 0.35rem 0.7rem; border: 1px solid rgba(0,0,0,0.06);">
                📈 Popularity ${popValue}
              </span>
              <span class="badge" style="background: rgba(255,255,255,0.95); backdrop-filter: blur(6px); color: #059669; font-weight: 800; font-size: 0.75rem; padding: 0.35rem 0.7rem; border: 1px solid rgba(0,0,0,0.06);">
                Avg. ${costStr}/day
              </span>
            </div>
          </div>
          ...
          <div style="padding: 1.15rem 1.4rem 1.4rem 1.4rem; background: #ffffff;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.35rem;">
              <h3 style="font-family: var(--font-heading); font-size: 1.35rem; font-weight: 900; color: #0f172a; line-height: 1.2;">
            ...
            <button 
              ...
              style="width: 100%; height: 46px; background: #006d64; color: #ffffff; border: none; border-radius: 9999px; font-size: 0.95rem; font-weight: 800; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 0.5rem; box-shadow: 0 4px 12px rgba(0, 109, 100, 0.25); transition: background 0.2s ease;"
            >
    ...
    destList.querySelectorAll('.btn-add-dest').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        // Current dates formatted YYYY-MM-DD
        const now = new Date();
        ...
        try {
          ...
          try {
            ...
            console.warn('API addStop fallback:', apiError.message);
            newStop = {
              id: 'stop-' + Date.now(),
              tripId,
              cityId,
